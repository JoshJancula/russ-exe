...
$(document).ready(() => {
    ...
    openWoundImages.addEventListener('click', () => {
        ipcRenderer.send('open-images', fakeImageData);
    });
    ...
    images.forEach(pic => {
        ...
        let newImage = `
                <div class="mySlides fade">
                    <div class="numbertext">${count} / ${images.length}</div>
                            <img src="${pic.src}" style="height: 400px; width:100%">
                <div class="text">${data ? pic.wound_desc : count}</div>
            </div>
      `;
